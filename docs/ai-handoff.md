...
- Bookings reserve room-category inventory and assign a physical room at check-in.
- Booking creation uses a PostgreSQL transaction-scoped advisory lock per property and room category to prevent concurrent last-slot oversell.
...
- Payments are recorded as transactions with adapter dispatch and idempotency-key support. Mock/local providers work; Razorpay/Stripe are explicit placeholders until credentials and webhook verification are implemented.
- Channel manager boundary supports connections, external room/rate mappings, sync logs, and adapter dispatch. Mock sync works; SiteMinder/Booking.com/Airbnb are explicit placeholders until provider contracts are implemented.
...
- Audit logs record booking creation, check-in, checkout, room create/update/delete, payment collection/refunds, channel setup/mappings, and channel sync outcomes.
...
- `booking`: booking creation, check-in, checkout, check-in reminder, paginated/searchable listing.
...
- `payment`: collect/refund payment transaction workflow with provider adapter dispatch and paginated/searchable listing.
- `channel`: channel connections, room/rate mappings, provider adapter dispatch, mock sync logs.
...
- Booking total is the sum of per-night dynamic rates built from the base rate plus any active pricing rules.
- Overlapping bookings are allowed only until category inventory is sold out.
- Active booking statuses for availability are `BOOKED` and `CHECKED_IN`.
- Concurrent booking creation for the same property and room category is serialized before inventory counting.
- Check-in assigns an available physical room and marks it `OCCUPIED`.
- Check-out marks booking `CHECKED_OUT` and room `AVAILABLE`.
...
- Add real channel adapters such as SiteMinder, Booking.com, and Airbnb.
...
5. Replace external provider placeholders with real provider-specific adapters when credentials/contracts are available.
