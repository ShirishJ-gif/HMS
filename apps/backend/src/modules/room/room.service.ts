...
export class RoomService {
  ...
  async getWorkspace(user?: AuthenticatedUser) {
    ...
    const [rooms, properties, categories, checkedInStays] = await this.prisma.$transaction([
      this.prisma.room.findMany({
    ...
    return {
      ...
      occupancy: checkedInStays.flatMap((stay) => stay.roomId
        ? [{
            room_id: stay.roomId,
            arrival_date: stay.arrivalDate.toISOString().slice(0, 10),
            departure_date: stay.departureDate.toISOString().slice(0, 10),
            external_reservation_id: stay.reservationGroup.externalReservationId,
            guest_name: stay.guestName ?? stay.reservationGroup.primaryGuest?.name ?? 'Imported guest',
          }]
        : []),
    };
  }
